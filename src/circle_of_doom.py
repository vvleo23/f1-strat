"""Erzeugt ein interaktives OpenF1-Circle-of-Doom-Replay.

Die Kreispositionen zeigen den aus OpenF1-Koordinaten abgeleiteten geometrischen
Fortschritt innerhalb der aktuellen Runde: oben ist Start/Ziel, unten die halbe
Rundendistanz. Für einen ausgewählten Fahrer wird zusätzlich die hypothetische
Position nach sofortigem Boxenstopp dargestellt.
"""
...
# Community-Limit: höchstens 30 Requests pro Minute.
...
class CircleOfDoomError(RuntimeError):
    """Beschreibt einen verständlichen Fehler beim Erstellen des Replays."""


@dataclass(frozen=True)
class CarState:
    """Zeitpunktbezogener Zustand eines Fahrzeugs im Replay."""
...
@dataclass(frozen=True)
class PitProjection:
    """Hypothetische Boxenausfahrt eines ausgewählten Fahrers."""
...
@dataclass(frozen=True)
class ReplayFrame:
    """Alle darzustellenden Daten eines Zeitpunkts."""
...
class OpenF1Client:
    ...
    def __init__(self) -> None:
        ...
        self.session.headers.update({"User-Agent": "f1-api-validation-circle-of-doom/1.0"})
    ...
    def _enable_system_trust_store() -> None:
        try:
            ...
            raise CircleOfDoomError(
                "Das Paket 'truststore' fehlt. Bitte 'pip install -r requirements.txt' ausführen."
            ) from exc
        truststore.inject_into_ssl()

    def get_json(self, endpoint: str, params: dict[str, Any]) -> list[dict[str, Any]]:
        """Ruft einen OpenF1-Endpunkt sicher ab und validiert die Antwort."""
        ...
        try:
            ...
            for attempt in range(3):
                ...
                if attempt < 2:
                    ...
                    print(
                        f"location temporär abgelehnt; erneuter Versuch in "
                        f"{wait_seconds}s ..."
                    )
            ...
            raise CircleOfDoomError(
                "TLS-Zertifikatsprüfung für OpenF1 ist fehlgeschlagen. "
                "Der System-Zertifikatsspeicher konnte die Verbindung nicht validieren."
            ) from exc
            ...
            raise CircleOfDoomError(
                f"Der OpenF1-Endpunkt '{endpoint}' lieferte kein gültiges JSON."
            ) from exc
        except requests.exceptions.HTTPError as exc:
            status = exc.response.status_code if exc.response is not None else "unbekannt"
            raise CircleOfDoomError(
                f"OpenF1 lieferte HTTP {status} für den Endpunkt '{endpoint}'."
            ) from exc
        ...
        if not isinstance(payload, list) or not all(isinstance(row, dict) for row in payload):
            raise CircleOfDoomError(
                f"Unerwartetes Antwortformat des OpenF1-Endpunkts '{endpoint}'."
            )
        return payload


def cache_path(session_key: int, endpoint: str) -> Path:
    """Liefert den Parquet-Cachepfad eines OpenF1-Datensatzes."""
...
def location_driver_cache_path(session_key: int, driver_number: int) -> Path:
    """Liefert den resumierbaren Location-Cachepfad eines Fahrers."""
    return RAW_DATA_DIR / (
...
def load_session_datasets(
    ...
    *,
    ...
    """Lädt alle Replay-Datensätze aus Cache oder OpenF1."""
    ...
    for endpoint in DATASET_ENDPOINTS:
        ...
        if path.exists() and not refresh:
            ...
            if endpoint == "location":
                ...
                for index, driver_number in enumerate(driver_numbers, start=1):
                    driver_path = location_driver_cache_path(
                    ...
                    print(
                        f"location: Fahrer {driver_number} "
                        f"({index}/{len(driver_numbers)}, {driver_source}, "
                        f"{len(driver_frame):,} Zeilen)"
                    )
        ...
        print(f"{endpoint}: {len(frame):,} Zeilen ({source})")
    ...
    if empty_required:
        raise CircleOfDoomError(
            "Erforderliche Datensätze sind leer: " + ", ".join(empty_required)
        )
    return datasets
...
def infer_race_window(
    ...
    """Ermittelt tatsächlichen Rennstart und Rennende aus Race Control/Laps."""
...
def estimate_reference_lap_time(laps: pd.DataFrame, pits: pd.DataFrame) -> float:
    """Schätzt eine robuste repräsentative Rundenzeit für die Kreisabbildung."""
    working = laps.copy()
    ...
    if valid.empty:
        raise CircleOfDoomError("Keine plausible Referenz-Rundenzeit verfügbar.")
...
def status_events(
    ...
    """Erzeugt eine kompakte Status-Zeitleiste für GREEN, SC, VSC und RED."""
...
def reconstruct_absolute_gaps(
    ...
    """Rekonstruiert absolute Zeitabstände inklusive überrundeter Fahrzeuge."""
...
def build_location_progress(location: pd.DataFrame, laps: pd.DataFrame) -> pd.DataFrame:
    """Berechnet je Location-Messung den geometrischen Fortschritt der Runde.

    Die Normierung verwendet die in derselben Runde beobachtete x/y/z-Weglänge.
    Sie dient ausschließlich der nachträglichen Visualisierung und fließt nicht
    in Strategie- oder Pit-Positionsberechnungen ein.
    """
    ...
    if not progress_parts:
        raise CircleOfDoomError(
            "Aus den OpenF1-Location-Daten konnte kein Rundenfortschritt berechnet werden."
        )
...
def _prepare_asof_grid(
    ...
    """Synchronisiert alle Eingänge auf eine gemeinsame Replay-Zeitachse."""
    ...
    # OpenF1 überträgt unveränderte Abstände teilweise erst nach etwa einer
    # Runde erneut (z. B. 0 s für den Führenden). Der letzte ausschließlich
    # aus der Vergangenheit bekannte Wert bleibt deshalb gültig. Die deutlich
    # höherfrequenten Location-Daten bestimmen separat, ob ein Auto noch aktiv
    # auf dem Kreis dargestellt werden kann.
...
def build_replay(
    ...
    *,
    ...
    """Rekonstruiert alle Race-Replay-Frames ohne zukünftige Messwerte."""
    if frame_seconds <= 0:
        raise ValueError("frame_seconds muss größer als null sein.")
    if max_staleness_seconds <= 0:
        raise ValueError("max_staleness_seconds muss größer als null sein.")
    if green_pit_loss < 0 or neutralized_pit_loss < 0:
        raise ValueError("Pit-Loss-Werte dürfen nicht negativ sein.")
    ...
    if focus_driver not in driver_meta:
        raise CircleOfDoomError(f"Fahrer {focus_driver} ist in dieser Session nicht vorhanden.")
    ...
    for date, snapshot in grid.groupby("date", sort=True):
        ...
        # Eine feste Reihenfolge ist für die browserseitige Interpolation wichtig:
...
def resolve_driver_number(drivers: pd.DataFrame, selector: str) -> int:
    """Löst Fahrernummer oder Akronym auf."""
    ...
    raise CircleOfDoomError(
        f"Fahrer '{selector}' wurde nicht eindeutig gefunden. Verfügbar: {available}"
    )
...
def _projection_description(projection: PitProjection | None) -> str:
    if projection is None:
        return "Keine aktuelle Projektion"
    neighbours: list[str] = []
    if projection.ahead:
        neighbours.append(f"{projection.gap_ahead:.1f}s hinter {projection.ahead}")
    if projection.behind:
        neighbours.append(f"{projection.gap_behind:.1f}s vor {projection.behind}")
    return " · ".join(neighbours) if neighbours else "Keine direkten Nachbarn"
...
def _frame_traces(
    ...
    for driver_number in ordered_drivers:
        ...
        if car is None:
            ...
            hovers.append("")
            continue
        car_x, car_y = _xy_for_progress(car.lap_progress)
        x.append(car_x)
        y.append(car_y)
        ...
        tyre = car.compound or "unbekannt"
        tyre_age = f"{car.tyre_age} Runden" if car.tyre_age is not None else "unbekannt"
        hovers.append(
            f"<b>P{car.position} {car.acronym}</b><br>"
            f"Abstand: {car.displayed_gap}<br>Runde: {car.lap_number}<br>"
            f"Rundenfortschritt: {car.lap_progress * 100:.1f}%<br>"
            f"Reifen: {tyre}, Alter: {tyre_age}"
        )
    ...
    if projection is None:
        projection_trace = go.Scattergl(
            x=[], y=[], mode="markers", name="Pit-Projektion"
        )
        arc_trace = go.Scatter(x=[], y=[], mode="lines", name="Pit-Loss")
    else:
        projection_x, projection_y = _xy_for_progress(
            projection.projected_progress, radius=1.08
        )
        projection_trace = go.Scattergl(
            x=[projection_x],
            y=[projection_y],
            ...
            hovertext=[
                f"<b>Projizierte Boxenausfahrt P{projection.projected_position}</b><br>"
                f"Pit Loss: {projection.pit_loss:.1f}s<br>"
                f"Abstand zum Führenden: +{projection.projected_gap:.1f}s<br>"
        ...
        arc_points = [
            _xy_for_progress(progress, radius=1.08) for progress in arc_progress
        ]
        arc_trace = go.Scatter(
            ...
            hoverinfo="skip",
            name="angenommener Pit Loss",
        )
    return cars_trace, projection_trace, arc_trace


def _frame_title(frame: ReplayFrame, focus_acronym: str) -> str:
    projection = frame.projection
    clock = frame.date.strftime("%H:%M:%S UTC")
    if projection:
        projection_text = (
            f"{focus_acronym}: sofortiger Stopp → P{projection.projected_position} "
            f"(+{projection.projected_gap:.1f}s; Pit Loss {projection.pit_loss:.1f}s)"
        )
    else:
        projection_text = f"{focus_acronym}: keine frische Abstandsmessung"
    return (
        f"Circle of Doom · Runde {frame.lap_number} · {clock} · {frame.status}"
        f"<br><sup>{projection_text} · {_projection_description(projection)}</sup>"
    )


def _replay_driver_order(replay: ReplayResult) -> tuple[int, ...]:
    """Liefert die kanonische Fahrerreihenfolge für alle Animationsframes."""
    return tuple(
...
def build_animation_post_script(
    replay: ReplayResult, *, frame_seconds: int
) -> str:
    """Erzeugt die WebGL-Interpolation und die Wiedergabesteuerung für das HTML."""
...
(() => {{
    ...
    function interpolateProgress(from, to, fraction) {{
        ...
        if (delta > 0.5) delta -= 1;
        // Kleine rückwärts gerichtete GPS-Korrekturen sollen kein Auto entgegen
        // der Fahrtrichtung oder fast eine ganze Runde vorwärts springen lassen.
    ...
    // Bewusste öffentliche Test-/Debug-Schnittstelle im erzeugten HTML.
...
def create_figure(
    ...
    *,
    ...
    """Erstellt die interaktive Plotly-Animation."""
    circle_angles = np.linspace(0, 2 * math.pi, 361)
    track_trace = go.Scatter(
        x=np.cos(circle_angles).tolist(),
        y=np.sin(circle_angles).tolist(),
        mode="lines",
    ...
    first_traces = _frame_traces(
        replay.frames[0], replay.reference_lap_time, focus_driver, driver_order
    )
    ...
    for index, frame in enumerate(replay.frames):
        ...
        traces = _frame_traces(
            frame, replay.reference_lap_time, focus_driver, driver_order
        )
        plotly_frames.append(
            go.Frame(
                ...
                traces=[3, 4, 5],
                name=name,
                layout=go.Layout(title={"text": _frame_title(frame, focus_acronym)}),
            )
        )
        slider_steps.append(
            {
    ...
    figure.update_layout(
        ...
        title={"text": _frame_title(replay.frames[0], focus_acronym), "x": 0.5},
        width=1000,
        height=900,
        showlegend=True,
        ...
        annotations=[
            {
                "text": (
                    "Position = geometrischer Rundenfortschritt aus x/y/z<br>"
                    "Zeitabstand = OpenF1 gap_to_leader<br>"
                    f"Pit-Projektion mit Referenzrunde {replay.reference_lap_time:.1f}s<br>"
                    "Diamant = kürzlich gestoppt"
                    f"<br>Keyframes alle {frame_seconds}s · flüssig interpoliert"
                ),
                "x": 0,
                "y": 0,
                "showarrow": False,
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Erzeugt ein interaktives Circle-of-Doom-Replay aus historischen OpenF1-Daten."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Fahrernummer oder Akronym für die Pit-Exit-Projektion (Standard: ANT).",
    )
    parser.add_argument(
        "--pit-loss",
        type=float,
        default=DEFAULT_GREEN_PIT_LOSS_SECONDS,
        help="Angenommener Zeitverlust unter Grün in Sekunden.",
    )
    parser.add_argument(
        "--neutralized-pit-loss",
        type=float,
        default=DEFAULT_NEUTRALIZED_PIT_LOSS_SECONDS,
        help="Angenommener Zeitverlust unter SC/VSC in Sekunden.",
    )
    parser.add_argument(
        "--frame-seconds",
        type=int,
        default=DEFAULT_FRAME_SECONDS,
        help="Zeitabstand zwischen Animationsframes.",
    )
    parser.add_argument(
        "--max-staleness",
        type=int,
        default=DEFAULT_MAX_STALENESS_SECONDS,
        help="Maximales Alter fortgeschriebener Location-Messungen in Sekunden.",
    )
    parser.add_argument("--refresh", action="store_true", help="OpenF1-Cache neu laden.")
    parser.add_argument(
        "--self-contained",
        action="store_true",
        help="Plotly-JavaScript in die HTML-Datei einbetten statt CDN zu verwenden.",
    )
    parser.add_argument("--open", action="store_true", help="Ergebnis im Browser öffnen.")
    parser.add_argument("--output", type=Path, help="Abweichender HTML-Ausgabepfad.")
    return parser


def main(argv: list[str] | None = None) -> int:
    """CLI-Einstiegspunkt für Download, Replay und HTML-Erzeugung."""
    ...
    try:
        ...
        figure.write_html(
            ...
            post_script=build_animation_post_script(
                replay, frame_seconds=args.frame_seconds
            ),
        )

        session = datasets["sessions"].iloc[0]
        print(
            f"\nCircle of Doom erstellt: {output}\n"
            f"Session: {session.get('circuit_short_name', args.session_key)} "
            f"{session.get('year', '')}\n"
            f"Fokus: {focus_acronym} ({focus_driver})\n"
            f"Frames: {len(replay.frames):,}\n"
            f"Referenz-Rundenzeit: {replay.reference_lap_time:.3f}s"
        )
        if args.open:
            webbrowser.open(output.as_uri())
        return 0
    except (CircleOfDoomError, KeyError, TypeError, ValueError, OSError) as exc:
        print(f"Circle-of-Doom-Fehler: {exc}", file=sys.stderr)
        return 1
